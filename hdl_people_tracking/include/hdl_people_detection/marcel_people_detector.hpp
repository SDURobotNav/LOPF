...
public:
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> detect_clusters(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& cloud) 
  {
    const float lambda = 0.45;
    ...
    for (int i = 0; i < candidate_clusters.size(); i++) 
    {
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr cluster_cloud(new pcl::PointCloud<pcl::PointXYZI>());
      extract.filter(*cluster_cloud);
      const int a = cluster_cloud->size();
      ROS_INFO("Size of cloud %d",a);
      if (a < 20) {
        continue;
      }
