...
public:
  ...
  void onInit() override {
    ...
    // flag_sub = nh.subscribe("/record_time", 1, &HdlPeopleDetectionNodelet::handle_flag, this);
...
private:
  ...
  void callback_static(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    std::string frameID = points_msg->header.frame_id;
    ros::Time timeStamp = points_msg->header.stamp;
    ...
    auto clusters = detector->detect(filtered,frameID,timeStamp);;
  ...
  //   ROS_INFO("HHHH");
  //   std::ofstream file;
  //   file.open("~/tracking_ws/result_cpp.txt");
  //   if(file.is_open())
  //   {
  //     ROS_INFO("Saving CPP Result!!!");
  //     for(std::vector<uint32_t>::iterator i = time_vec.begin(), end=time_vec.end();i!=end;++i)
  //     {
  //       std::cout<<*i<<std::endl;
  //       file<<(*i);
  //     }
...
private:
  ...
  void publish_msgs(const ros::Time& stamp, const pcl::PointCloud<pcl::PointXYZI>::Ptr& filtered, const std::vector<Cluster::Ptr>& clusters) const {
    ...
    if(detection_markers_pub.getNumSubscribers()) {
      ...
      ROS_INFO("Detection Markers size is %d",clusters.size());
    }
  }
